...
| Risk Budget (`goldenboy.core.risk_budget`): cumulative, point-based risk accounting, distinct from `RiskEngine`'s budget-ratio and `PolicyEngine`'s rule-matching | Done (2026-09-22) — `goldenboy risk` CLI command; monotonic (tightens only, never auto-loosens); operation weights configurable via `.goldenboy/risk_budget.json` |
| Failure classification (`goldenboy.core.failure_memory.FailureCategory`/`classify_failure`) | Done (2026-09-22) — deterministic, ordered-regex classifier over 9 categories; wired into `FailureMemoryStore.record()` and `goldenboy failure record --category` |
